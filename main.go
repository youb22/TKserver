...
import (
	...
	"gorm.io/gorm"
	"github.com/youb22/TKserver/models"
)
...
// Function to initialize the database connection and assign to the global variable
func initializeDatabase() {
    // Define the connection string (update with your credentials)
    connStr := "host=localhost user=app password=patapouf dbname=app sslmode=disable"
    var err error
...
// Function to insert a message into the database (using the global db variable)
func insertTkMessage(imei string, message string) error {
    query := `INSERT INTO "tk_message" (imei, message) VALUES ($1, $2)`
    _, err := db.Exec(query, imei, message)
    if err != nil {
...
func (r *TCPServer) handleConnection(conn net.Conn) {
    ...
    // TODO: Check in the DB if this imei exists
    // If it does then do nothing
    // If it doesn't then create a new row in the DB
	...
	for {
		...
		//Add an entry with the imei and message in the DB
		/* err_i := insertTkMessage(imei, hex.EncodeToString(readBuffer[:read]))
    		if err_i != nil {
        		log.Fatal(err_i)
    		}
        */
